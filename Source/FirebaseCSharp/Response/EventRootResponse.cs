using FirebaseCSharp.EventStreaming;
...
namespace FirebaseCSharp.Response
{
    public class EventRootResponse<T> : IDisposable
    {
        ...
        private async Task ReadLoop(HttpResponseMessage httpResponse, CancellationToken token)
        {
            ...
            {
                using (httpResponse)
                await using (Stream content = await httpResponse.Content.ReadAsStreamAsync().ConfigureAwait(false))
                using (StreamReader sr = new StreamReader(content))
                {
                    ...
                    while (true)
                    {
                        ...
                        if (read != null && read.StartsWith("data: "))
                        {
                            ...
                            string jsonStr = await request.Content.ReadAsStringAsync().ConfigureAwait(false);
